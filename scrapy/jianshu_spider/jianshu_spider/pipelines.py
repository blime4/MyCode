...
class JianshuSpiderPipeline(object):
    def __init__(self):
        dbparmas = {
            ...
            'user': 'blime',
            'password': 'pls',
            'database': 'scrapy.jianshu_article',
            'charset': 'utf8'
        }
    ...
    def sql(self):
        if not self._sql:
            self._sql = """
            insert into scrapy.jianshu_article(id,title,content,author,avatar,pub_time,origin_url,article_id) values(null,%s,%s,%s,%s,%s,%s)
            """
            return self._sql
        return self._sql
